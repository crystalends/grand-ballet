...
const EXPECTED_PAGE_HEIGHT = 6185;
const MAX_DIFF_RATIO = 0.035;

const expectedSections = {
  ".college-header": { x: 190, y: 0, width: 1540, height: 126 },
  ".director-hero": { x: 190, y: 146, width: 1540, height: 600 },
  ".director-story": { x: 190, y: 786, width: 1540, height: 574 },
  ".director-journey": { x: 190, y: 1460, width: 1540, height: 478 },
  ".director-role": { x: 190, y: 2038, width: 1540, height: 589 },
  ".director-potential": { x: 190, y: 2727, width: 1540, height: 489 },
  ".director-route": { x: 190, y: 3316, width: 1540, height: 465 },
  ".director-gallery": { x: 190, y: 3881, width: 1540, height: 387 },
  ".faq": { x: 190, y: 4368, width: 1540, height: 542 },
  ".seo-copy": { x: 190, y: 5010, width: 1540, height: 208 },
  ".trial": { x: 190, y: 5318, width: 1540, height: 478 },
  ".college-footer": { x: 190, y: 5836, width: 1540, height: 349 },
};
...
try {
  ...
  await page.goto(`${BASE_URL}/director.html`, { waitUntil: "networkidle" });
  ...
  const galleryImages = page.locator(".director-gallery__image");
  if (await galleryImages.count() !== 3) throw new Error("В галерее должно быть три изображения из Figma.");
  if (await page.locator(".director-potential-card").count() !== 4) throw new Error("В блоке потенциала должно быть четыре карточки.");
  if (await page.locator(".director-role__tasks li").count() !== 6) throw new Error("В списке задач директора должно быть шесть пунктов.");
  const routeCards = page.locator(".director-route .mission-card");
  if (await routeCards.count() !== 4) throw new Error("Маршрут должен переиспользовать четыре mission-card.");
  const routeLines = await routeCards.locator(".mission-card__line").evaluateAll((images) => images.map((image) => image.getAttribute("src")));
  if (routeLines.some((source) => source !== "assets/images/service/levels-line.svg")) throw new Error("Маршрут должен переиспользовать общий levels-line.svg.");
  if (await routeCards.locator(".mission-card__line-frame--route").count() !== 4) throw new Error("SVG-линия маршрута должна позиционироваться через общий line-frame.");

  await mkdir("artifacts", { recursive: true });
  const actualPath = "artifacts/director-actual.png";
  const diffPath = "artifacts/director-diff.png";
  await page.screenshot({ path: actualPath, fullPage: true });
  const [referenceBuffer, actualBuffer] = await Promise.all([readFile("assets/reference/figma-director-page.png"), readFile(actualPath)]);
  ...
  console.log(`Director visual diff: ${(diffRatio * 100).toFixed(2)}%`);
  ...
  console.log("Director geometry, visual, FAQ, form and 390px mobile audit: OK");
