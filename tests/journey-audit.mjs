...
const EXPECTED_SIZE = { width: 1540, height: 780 };
...
try {
  ...
  const journey = page.locator(".journey__grid");
  const box = await journey.boundingBox();
  if (!box || box.width !== EXPECTED_SIZE.width || box.height !== EXPECTED_SIZE.height) {
    throw new Error(`Неверная геометрия пути ученика: ${box?.width}×${box?.height}.`);
  }

  await mkdir("artifacts", { recursive: true });
  const actualPath = "artifacts/journey-actual.png";
  const diffPath = "artifacts/journey-diff.png";
  await journey.screenshot({ path: actualPath, animations: "disabled" });

  const [referenceBuffer, actualBuffer] = await Promise.all([
    readFile("assets/reference/figma-journey.png"),
    readFile(actualPath),
  ]);
  ...
  console.log(`Journey visual diff: ${(diffRatio * 100).toFixed(2)}%`);

  if (consoleErrors.length) throw new Error(`Ошибки консоли: ${consoleErrors.join(" | ")}`);
  if (diffRatio > MAX_DIFF_RATIO) {
    throw new Error(`Расхождение пути ученика ${(diffRatio * 100).toFixed(2)}% превышает лимит ${MAX_DIFF_RATIO * 100}%.`);
  }
} finally {
  await browser?.close();
  server?.kill();
