...
const EXPECTED_SIZE = { width: 1540, height: 710 };
...
try {
  ...
  const box = await courses.boundingBox();
  if (!box || box.width !== EXPECTED_SIZE.width || box.height !== EXPECTED_SIZE.height) {
    throw new Error(`Неверная геометрия секции занятий: ${box?.width}×${box?.height}.`);
  }

  await mkdir("artifacts", { recursive: true });
  const actualPath = "artifacts/courses-actual.png";
  const diffPath = "artifacts/courses-diff.png";
  await courses.screenshot({ path: actualPath, animations: "disabled" });

  const [referenceBuffer, actualBuffer] = await Promise.all([
    readFile("assets/reference/figma-courses.png"),
    readFile(actualPath),
  ]);
  ...
  console.log(`Courses visual diff: ${(diffRatio * 100).toFixed(2)}%`);

  if (consoleErrors.length) throw new Error(`Ошибки консоли: ${consoleErrors.join(" | ")}`);
  if (diffRatio > MAX_DIFF_RATIO) {
    throw new Error(`Расхождение секции занятий ${(diffRatio * 100).toFixed(2)}% превышает лимит ${MAX_DIFF_RATIO * 100}%.`);
  }
} finally {
  await browser?.close();
  server?.kill();
