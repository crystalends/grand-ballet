...
const EXPECTED_PAGE_HEIGHT = 6940;
const MAX_DIFF_RATIO = 0.04;

const expectedSections = {
  ".college-header": { x: 190, y: 0, width: 1540, height: 126 },
  ".preparation-hero": { x: 190, y: 136, width: 1540, height: 566 },
  ".preparation-audience": { x: 190, y: 802, width: 1540, height: 498 },
  ".preparation-program": { x: 190, y: 1400, width: 1540, height: 370 },
  ".preparation-outcomes": { x: 190, y: 1870, width: 1540, height: 585 },
  ".preparation-format": { x: 190, y: 2555, width: 1540, height: 706 },
  ".teachers": { x: 150, y: 3361, width: 1620, height: 573 },
  ".teachers .slider-shell__inner": { x: 190, y: 3361, width: 1540, height: 573 },
  ".preparation-exams": { x: 190, y: 4034, width: 1540, height: 559 },
  ".preparation-calm": { x: 190, y: 4693, width: 1540, height: 330 },
  ".faq": { x: 190, y: 5123, width: 1540, height: 542 },
  ".seo-copy": { x: 190, y: 5765, width: 1540, height: 208 },
  ".trial": { x: 190, y: 6073, width: 1540, height: 478 },
  ".college-footer": { x: 190, y: 6591, width: 1540, height: 349 },
};
...
try {
  ...
  await page.goto(`${BASE_URL}/preparation.html`, { waitUntil: "networkidle" });
  ...
  if (await page.locator(".preparation-audience .college-principle").count() !== 4) throw new Error("В блоке аудитории должно быть четыре переиспользованных college-principle.");
  if (await page.locator(".preparation-format .admission-step").count() !== 5) throw new Error("Формат подготовки должен переиспользовать пять admission-step и изображение.");
  if (await page.locator(".teachers .teacher-card").count() < 5) throw new Error("Карусель педагогов заполнена не полностью.");

  await mkdir("artifacts", { recursive: true });
  const actualPath = "artifacts/preparation-actual.png";
  const diffPath = "artifacts/preparation-diff.png";
  await page.screenshot({ path: actualPath, fullPage: true });
  const [referenceBuffer, actualBuffer] = await Promise.all([readFile("assets/reference/figma-preparation-page-1920.png"), readFile(actualPath)]);
  ...
  console.log(`Preparation visual diff: ${(diffRatio * 100).toFixed(2)}%`);
  ...
  console.log("Preparation geometry, visual, carousel, FAQ, form and 390px mobile audit: OK");
